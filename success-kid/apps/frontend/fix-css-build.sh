#!/bin/bash
# Fix CSS build issues by installing missing dependencies and updating config
...
npm install --save-dev mini-css-extract-plugin css-loader postcss-loader
...
# Update next.config.js to include mini-css-extract-plugin
...
const nextConfig = {
  ...
  webpack: (config, { dev, isServer }) => {
    ...
    // Add CSS extraction configuration
    if (!isServer) {
      ...
      // Find the CSS rule and update its use configuration
      const cssRule = config.module.rules.find(
        rule => rule.test && rule.test.toString().includes('css')
      );
      
      if (cssRule) {
        // Ensure CSS rule is configured correctly
        cssRule.use = [
        ...
        // If no existing CSS rule, add one
        config.module.rules.push({
          test: /\.css$/,
          use: [
            MiniCssExtractPlugin.loader,
...
echo "CSS build configuration fixed successfully!"
