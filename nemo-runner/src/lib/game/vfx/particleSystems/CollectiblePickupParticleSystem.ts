...
export class CollectiblePickupParticleSystem {
  ...
  constructor(private scene: THREE.Scene, private shaderManager: ShaderManager) {
    const cfg = (configSystem.get('visuals') as any).collectibleSparks || { enabled: false, poolSize: 50 };
  ...
  private initialize(): void {
    ...
    for (let i = 0; i < this.poolSize; i++) {
      ...
      this.lifetimes[i] = 0;
      this.particles[i] = new THREE.Vector3();
    }
  }
  ...
  public emit(origin: THREE.Vector3, color?: THREE.Color, count: number = 10): void {
    ...
    for (let c = 0; c < count; c++) {
      ...
      this.lifetimes[index] = 0.5; // half second
    }
  ...
  public update(deltaTime: number): void {
    ...
    for (let i = 0; i < this.poolSize; i++) {
      if (this.lifetimes[i] > 0) {
        ...
        this.positions[i*3+1] += deltaTime; // simple upward motion
        if (this.lifetimes[i] <= 0) {
          this.positions[i*3+1] = -9999;
        }
        needsUpdate = true;
