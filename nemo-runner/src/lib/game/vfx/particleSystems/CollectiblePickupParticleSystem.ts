...
export class CollectiblePickupParticleSystem {
  ...
  constructor(private scene: THREE.Scene, private shaderManager: ShaderManager) {
    const cfg = configSystem.get('visuals').collectibleSparks;
  ...
  private initGeometry(): void {
    ...
    g.setAttribute('aVelocity', new THREE.BufferAttribute(this.arrays.velocities, 3));
    g.setAttribute('aLifetime', new THREE.BufferAttribute(this.arrays.lifetimes, 1));
    ...
    const mat = this.shaderManager.createShaderMaterial('sparkleShader', {
    ...
    if (!mat) {
      console.error('CollectiblePickupParticleSystem: sparkleShader failed');
      this.material = new THREE.PointsMaterial({ color: 0xffff00, size: 0.1, transparent: true });
    } else {
      this.material = mat;
  ...
  public emit(origin: THREE.Vector3, particleColor?: THREE.Color): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const angle = Math.random() * (cfg.spreadAngle ?? 360) * THREE.MathUtils.DEG2RAD;
      ...
      this.arrays.velocities[idx * 3] = Math.cos(angle) * xzSpeed;
      this.arrays.velocities[idx * 3 + 1] = ySpeed;
      this.arrays.velocities[idx * 3 + 2] = Math.sin(angle) * xzSpeed;
