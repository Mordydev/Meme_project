# Success Kid Community Platform - Masterplan
...
* **Redemption Rate**: 100 SP = 10 SKC, with a daily cap (10,000 SP/user) to manage supply flow
...
|-----------|----------------|-------------------|
| **Daily Redemption Caps** | Maximum 10,000 SP (1,000 SKC) per user per day | Real-time cap enforcement, utilization tracking |
...
- **Conversion Process**:
  - Fixed rate: 100 SP = 10 SKC
  - Weekly redemption cap: 10,000 SP (1,000 SKC) per user
  - Minimum redemption: 1,000 SP (100 SKC)
...
- **Implementation**:
  ...
  function requestRedemption(userId, pointsAmount) {
    ...
    // Calculate token amount (10% of points)
    const tokenAmount = pointsAmount / 10;
...
- **Serverless First**: Leveraging serverless technologies for scalability and cost efficiency
- **Edge-optimized**: Utilizing edge functions and CDN for global performance
...
| Backend | Node.js 22.3, Next.js API Routes/Server Actions, TypeScript 5.4 | High-performance server, type safety, aligned with frontend |
| Real-time | Socket.io 4.x, Redis Streams 8.2 | Efficient WebSocket implementation with reliable message delivery |
| Database | Neon PostgreSQL (serverless), Drizzle ORM | Serverless relational database with type-safe ORM |
| Storage | Vercel Blob | Scalable, secure file storage with CDN integration |
...
| Infrastructure | Vercel, Cloudflare, Redis | Scalable, high-performance hosting with global CDN and edge computing |
...
The backend leverages Next.js API Routes and Server Actions with a modular structure:
...
│   │   └── avatars/          # User avatar processing
│   │
│   └── jobs/                 # Background processing
│       ├── points/           # Points calculation jobs
│       └── redemption/       # Token redemption jobs
│
├── test/                     # Test files
└── package.json              # Package dependencies
```
...
- **Vercel Blob Storage**:
  - User-generated content (images, media)
  - Profile pictures and avatars
  - Achievement badges and graphics
  - Content attachments
...
- **Database Optimization**:
  ...
  - Read replicas for high-traffic scenarios (future scale)

### CDN and Edge Optimization

- **Edge Functions**:
  - Critical operations at the edge
...
│   ├── web/                   # Next.js application (both frontend and backend)
...
| Action | Description | Authentication | Input Validation |
|--------|-------------|----------------|------------------|
| `createContent` | Create new platform content | Required | Zod schema |
| `addComment` | Add comment to content | Required | Zod schema |
| `redeemPoints` | Convert points to tokens | Required | Zod schema with caps |
| `connectWallet` | Connect user wallet | Required | Address validation |
| `updateProfile` | Update user profile | Required | Zod schema |
| `createReferral` | Generate referral link | Required | None |
| `interactWithContent` | Like, share, save content | Required | Action validation |
...
This plan provides clear, actionable guidance while maintaining flexibility for implementation teams. Success will be measured not just by user numbers, but by the quality of engagement and the strength of the community we build.
